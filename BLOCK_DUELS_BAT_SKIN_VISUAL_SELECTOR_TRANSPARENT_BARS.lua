...
;(function()
    ...
    do
        ...
        row.Parent = p
        addCorner(row, 10)
        local rowStroke = addStroke(row, Color3.fromRGB(35,35,35), 1)
...
-- ============================================================
-- BAT SKIN SELECTOR (VISUALS)
-- Matches the AVATAR selector card style
-- ============================================================
do
    local p = tabPages["VISUALS"]
    if p then
        ...
        local topRight = Instance.new("TextLabel", row)
        topRight.Name = "SelectedBatSkinLabel"
        topRight.Size = UDim2.new(0.5, -12, 0, 26)
        topRight.Position = UDim2.new(0.5, 0, 0, 7)
        topRight.BackgroundTransparency = 1
        topRight.Text = string.upper(currentBatSkin or "KATANA")
        topRight.TextColor3 = Color3.fromRGB(255,255,255)
        topRight.TextSize = 14
        topRight.Font = Enum.Font.GothamBlack
        topRight.TextXAlignment = Enum.TextXAlignment.Right
        topRight.ZIndex = 6
        batSkinStatusLabel = topRight
        ...
        local function setCardState(card, active)
            local bg = card:FindFirstChild("CardBackground")
            local stroke = card:FindFirstChild("CardStroke")
            local name = card:FindFirstChild("BatSkinName")
            local glyph = card:FindFirstChild("BatSkinPreview")
            if bg then
                tween(bg, {
                    BackgroundColor3 = active and Color3.fromRGB(42,42,46) or Color3.fromRGB(3,3,3),
                    BackgroundTransparency = active and 0.02 or 0.08,
            ...
            if glyph then
                tween(glyph, {
                    TextColor3 = active and Color3.fromRGB(255,255,255) or Color3.fromRGB(165,165,165)
                }, 0.12)
            end
        end

        refreshBatSkinCards = function()
            for skinName, card in pairs(cardButtons) do
                setCardState(card, currentBatSkin == skinName)
            end
            if batSkinStatusLabel then
                batSkinStatusLabel.Text = string.upper(currentBatSkin or "KATANA")
            end
        end

        for i, skinName in ipairs(BAT_SKIN_ORDER) do
            local card = Instance.new("TextButton", cardHolder)
            card.Name = "BatSkinCard_" .. i
            card.Size = UDim2.new(0, 78, 0, 142)
            card.BackgroundTransparency = 1
            card.BorderSizePixel = 0
            card.Text = ""
            ...
            local preview = Instance.new("TextLabel", card)
            preview.Name = "BatSkinPreview"
            preview.Size = UDim2.new(1, -8, 0, 103)
            preview.Position = UDim2.new(0, 4, 0, 4)
            preview.BackgroundTransparency = 1
            preview.Text = previewGlyph[skinName] or "BAT"
            preview.TextColor3 = Color3.fromRGB(165,165,165)
            preview.TextSize = 34
            preview.Font = Enum.Font.GothamBlack
            preview.TextXAlignment = Enum.TextXAlignment.Center
            preview.TextYAlignment = Enum.TextYAlignment.Center
            preview.ZIndex = 7

            local name = Instance.new("TextLabel", card)
            name.Name = "BatSkinName"
            name.Size = UDim2.new(1, -6, 0, 26)
            name.Position = UDim2.new(0, 3, 1, -30)
            name.BackgroundTransparency = 1
            name.Text = skinName
            ...
            cardButtons[skinName] = card
            batSkinCardRefs[skinName] = card

            card.MouseEnter:Connect(function()
                if currentBatSkin ~= skinName then
                    tween(cardBg, {BackgroundColor3 = Color3.fromRGB(20,20,22)}, 0.1)
                end
            end)
        ...
        local savedSkin = _configState["BatSkin"] or _configState["batSkin"]
        if savedSkin and table.find(BAT_SKIN_ORDER, tostring(savedSkin):upper()) then
            currentBatSkin = tostring(savedSkin):upper()
            if currentBatSkin == "BANHAMMER" then currentBatSkin = "BAN HAMMER" end
        end

        refreshBatSkinCards()

        _configState["BatSkin"] = currentBatSkin
        _configState["batSkin"] = currentBatSkin
        _autoSaveConfig()
