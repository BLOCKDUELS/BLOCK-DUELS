...
;(function()
    ...
    store.BackgroundImage.Position = UDim2.new(0, 0, 0, 0)
    store.BackgroundImage.BackgroundTransparency = 1
    store.BackgroundImage.BorderSizePixel = 0
    store.BackgroundImage.Image = _G.AdaptMobileButtonImage or ""
    store.BackgroundImage.ImageTransparency = 0.18
    store.BackgroundImage.ScaleType = Enum.ScaleType.Crop
    store.BackgroundImage.ZIndex = 51
    store.BackgroundImage.Parent = store.StealBar
    addCorner(store.BackgroundImage, 16)

    _G.ApplyStealBarBackgroundImage = function(image)
        if store.BackgroundImage and store.BackgroundImage.Parent then
            store.BackgroundImage.Image = tostring(image or "")
            store.BackgroundImage.Visible = image ~= nil and tostring(image) ~= ""
        end
    end

    if _G.AdaptMobileButtonImage then
        pcall(function()
            _G.ApplyStealBarBackgroundImage(_G.AdaptMobileButtonImage)
        end)
    end
...
;(function()
    ...
    do
        ...
        local function applyButtonBackground(index)
            ...
            _G.StealBarBackgroundImage = image
            if _G.ApplyStealBarBackgroundImage then
                pcall(function() _G.ApplyStealBarBackgroundImage(image) end)
            end
...
;(function()
    ...
    local Row = Instance.new("Frame")
    Row.Name = "BatSkinSelector"
    Row.Size = UDim2.new(1, 0, 0, 202)
    Row.BackgroundTransparency = 1
    Row.BorderSizePixel = 0
    Row.LayoutOrder = 36.5
    Row.Parent = p
    addCorner(Row, 10)
    addStroke(Row, Color3.fromRGB(55,55,55), 1)

    local topLeft = Instance.new("TextLabel")
    topLeft.Name = "BatSkinTitle"
    topLeft.Size = UDim2.new(0.5, -12, 0, 26)
    topLeft.Position = UDim2.new(0, 12, 0, 7)
    topLeft.BackgroundTransparency = 1
    topLeft.Text = "BAT SKIN"
    topLeft.TextColor3 = Color3.fromRGB(255,255,255)
    topLeft.TextSize = 15
    topLeft.Font = Enum.Font.GothamBlack
    topLeft.TextXAlignment = Enum.TextXAlignment.Left
    ...
    selected.Name = "SelectedBatSkinLabel"
    selected.Size = UDim2.new(0.5, -12, 0, 26)
    selected.Position = UDim2.new(0.5, 0, 0, 7)
    selected.BackgroundTransparency = 1
    selected.Text = tostring(currentBatSkin or "KATANA")
    ...
    cardHolder.Name = "BatSkinCards"
    cardHolder.Size = UDim2.new(1, -20, 0, 145)
    cardHolder.Position = UDim2.new(0, 10, 0, 42)
    cardHolder.BackgroundTransparency = 1
    cardHolder.ZIndex = 5
    cardHolder.Parent = Row
    ...
    local cardButtons = {}
    local cardData = {
        {skin = "KATANA", label = "KATANA"},
        {skin = "BAN HAMMER", label = "BAN HAMMER"},
        {skin = "SCYTHE", label = "SCYTHE"},
    }

    local function setCardState(card, active)
        local bg = card:FindFirstChild("CardBackground")
        local stroke = card:FindFirstChild("CardStroke")
        local name = card:FindFirstChild("SkinName")
        if bg then
            tween(bg, {
                BackgroundColor3 = active and Color3.fromRGB(42,42,46) or Color3.fromRGB(3,3,3),
                BackgroundTransparency = active and 0.02 or 0.08,
        ...
        if name then
            tween(name, {
                TextColor3 = active and Color3.fromRGB(255,255,255) or Color3.fromRGB(205,205,205)
            }, 0.12)
        end
    end

    function refreshBatSkinCards()
        local activeSkin = tostring(currentBatSkin or "KATANA"):upper()
        selected.Text = activeSkin
        for i, data in ipairs(cardData) do
            if cardButtons[i] then
                setCardState(cardButtons[i], data.skin == activeSkin)
            end
        end
    end
    ...
    for i, data in ipairs(cardData) do
        ...
        card.Name = "BatSkin_" .. tostring(i)
        card.Size = UDim2.new(0, 91, 0, 132)
        card.BackgroundTransparency = 1
        card.BorderSizePixel = 0
        card.AutoButtonColor = false
        card.Text = ""
        ...
        icon.Name = "SkinIcon"
        icon.Size = UDim2.new(1, -10, 0, 72)
        icon.Position = UDim2.new(0, 5, 0, 8)
        icon.BackgroundTransparency = 1
        icon.Text = (data.skin == "KATANA" and "⚔") or (data.skin == "SCYTHE" and "☾") or "⚒"
        ...
        name.Position = UDim2.new(0, 4, 1, -42)
        name.BackgroundTransparency = 1
        name.Text = data.label
        name.TextColor3 = Color3.fromRGB(205,205,205)
        name.TextSize = data.skin == "BAN HAMMER" and 10 or 11
        ...
        card.MouseButton1Click:Connect(function()
            currentBatSkin = data.skin
            selected.Text = data.label
            refreshBatSkinCards()
            pcall(function() applyBatSkin(data.skin) end)
            _configState["BatSkin"] = data.skin
            _autoSaveConfig()
        end)
    end

    local saved = loadConfig()
    local savedSkin = tostring(saved["BatSkin"] or currentBatSkin or "KATANA"):upper()
    if savedSkin == "BANHAMMER" then savedSkin = "BAN HAMMER" end
    if table.find(BAT_SKIN_ORDER, savedSkin) then
        currentBatSkin = savedSkin
    end

    refreshBatSkinCards()

    task.delay(0.8, function()
        if currentBatSkin and table.find(BAT_SKIN_ORDER, currentBatSkin) then
            pcall(function() applyBatSkin(currentBatSkin) end)
        end
    end)
