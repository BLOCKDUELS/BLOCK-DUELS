...
;(function()
    ...
    for i, skinName in ipairs(BAT_SKIN_ORDER) do
        ...
        icon.Name = "BatIcon"
        icon.Size = UDim2.new(1, -8, 0, 103)
        icon.Position = UDim2.new(0, 4, 0, 4)
        icon.BackgroundColor3 = Color3.fromRGB(0,0,0)
        icon.BackgroundTransparency = 0
        icon.BorderSizePixel = 0
        icon.Text = (skinName == "KATANA" and "⚔") or (skinName == "SCYTHE" and "☠") or "🔨"
        ...
        card.MouseButton1Click:Connect(function()
            local now = os.clock()
            currentBatSkin = skinName
            _configState["batSkin"] = skinName
            _autoSaveConfig()
            refreshBatSkinCards()
            task.spawn(function()
                pcall(function() applyBatSkin(skinName) end)
            end)
            lastTapCard = card
            lastTapTime = now
        end)
    ...
    local saved = loadConfig()
    local savedSkin = tostring(saved["batSkin"] or "KATANA"):upper()
